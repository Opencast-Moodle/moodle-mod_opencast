...
use block_opencast\local\upload_helper as block_upload_helper;
...
class upload_helper {
    ...
    private static function convert_metadatacatalog(
        ...
        if (get_string_manager()->string_exists($field->name, 'block_opencast')) {
            $label = get_string($field->name, 'block_opencast');
        }
        $params = [];
        if ($field->param_json) {
        ...
        if ($datatype == 'autocomplete') {
            $attributes = [
                'multiple' => true,
                'placeholder' => get_string('metadata_autocomplete_placeholder', 'block_opencast', $label),
                'showsuggestions' => true,
                'noselectionstring' => get_string('metadata_autocomplete_noselectionstring', 'block_opencast', $label),
                'tags' => true,
            ];
    ...
    private static function get_advanced_tab_presentation_config(int $ocinstanceid): array {
        ...
        if ($usechunkupload) {
            ...
            $uploadfsizelimit = (int) get_config('block_opencast', 'uploadfilesizelimitmode_' . $ocinstanceid);
    ...
    private static function get_advanced_tab_visibility_config(int $ocinstanceid, int $ocmoduleinstanceid): array {
        ...
        $params = get_string('visibility_hide', 'block_opencast');
        $attributes = 0;
        $default = $defaultvisibility;
        $config[] = new advancedupload_field($datatype, $id, $label, $params, $attributes, $default);
        $params = get_string('visibility_show', 'block_opencast');
        $label = '';
        $attributes = 1;
        $config[] = new advancedupload_field($datatype, $id, $label, $params, $attributes, $default);
        return $config;
    ...
    private static function get_advanced_tab_subtitle_config(int $ocinstanceid): array {
        ...
        // Prepare the subtitle settings check from block plugin.
        $sublangconfig = get_config('block_opencast', 'transcriptionlanguages_' . $ocinstanceid);
        $subuploadenabled = (bool) get_config('block_opencast', 'enableuploadtranscription_' . $ocinstanceid);
        ...
        $subtitletypescfg = get_config('block_opencast', 'transcriptionfileextensions_' . $ocinstanceid);
        ...
        foreach ($configedlanguages as $lang) {
            ...
            $label = !empty($lang->value) ? format_string($lang->value, true) :
                    get_string('transcriptionfilefield', 'block_opencast', $lang->key);
    ...
    public static function get_metadatacatalogs(int $ocinstanceid, bool $requiredonly = false, bool $optionalonly = false): array {
        ...
        $metadatacatalogs = block_upload_helper::get_opencast_metadata_catalog($ocinstanceid);
        $requiredmtcats = [];
        $optionalmtcats = [];
     ...
     * @throws \block_opencast\opencast_state_exception
     */
