...
/**
 * Inplace editable checkbox processing class for Opencast Activity.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class inplace_edit_checkbox_processing extends \core\output\inplace_editable {
    /**
     * Constructor.
     *
     * @param object $data
    ...
    public function __construct($data) {
        ...
        parent::__construct(
            ...
            itemtype: 'inplace_edit_checkbox_processing',
            itemid: $itemid,
            ...
            value: $data->value,
            edithint: get_string('uploadform_inplace_edit_checkbox_hint', 'mod_opencast'),
            editlabel: get_string('uploadform_inplace_edit_checkbox_label', 'mod_opencast')
        );
    ...
    public function export_for_template(\renderer_base $output) {
        if ($this->value) {
            $this->edithint = get_string('uploadform_inplace_edit_checkbox_true_hint', 'mod_opencast');
            $this->displayvalue = $output->pix_icon('i/checked', $this->edithint, '', ['class' => 'text-primary']);
        } else {
            $this->edithint = get_string('uploadform_inplace_edit_checkbox_false_hint', 'mod_opencast');
            $this->displayvalue = $output->pix_icon('e/cancel', $this->edithint, '', ['class' => 'text-primary']);
        }
    ...
    public static function update($itemid, $newvalue) {
        ...
        $uploadoptionsjson->options->{$uploadoptionsjson->selectedocinstanceid}->processing->{$id} = $newvalue ? 'true' : 'false';
