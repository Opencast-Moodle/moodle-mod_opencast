...
/**
 * Inplace editable autocomplete metadata class for Opencast Activity.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class inplace_edit_autocomplete_metadata extends \core\output\inplace_editable {
    /** @var string */
    private $name = null;
    /**
     * Constructor.
    ...
    public function __construct($data) {
        ...
        if (!empty($data->value)) {
            ...
            $value = !is_array($data->value) ? json_decode(json_encode((array) $data->value), true) : $data->value;
        ...
        parent::__construct(
            ...
            itemtype: 'inplace_edit_autocomplete_metadata',
            itemid: $itemid,
            ...
            value: $value,
            edithint: get_string('uploadform_inplace_edit_autocomplete_hint', 'mod_opencast'),
            editlabel: get_string('uploadform_inplace_edit_autocomplete_label', 'mod_opencast')
        );
    ...
    public function export_for_template(\renderer_base $output) {
        ...
        if (!empty($values)) {
            $this->displayvalue = implode(', ', $values);
        } else {
            $this->displayvalue = get_string('metadata_autocomplete_placeholder', 'block_opencast', $this->name);
        }
